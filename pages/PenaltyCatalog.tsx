...
const PenaltyCatalog: React.FC = () => {
  ...
  const handleAddPenalty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchPenaltyCatalog(); // Katalog neu laden
    ...
    <div className="container mx-auto mt-24 p-4">
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        {catalogItems.length === 0 ? (
          <p className="text-gray-600">Noch keine Strafen im Katalog dieses Teams.</p>
        ) : (
      ...
      {/* Modal für Strafe hinzufügen */}
      <Modal isOpen={isAddPenaltyModalOpen} onClose={() => setIsAddPenaltyModalOpen(false)} title="Neue Strafe zum Katalog hinzufügen">
        <form onSubmit={handleAddPenalty} className="space-y-4">
          ...
          <button
            ...
            disabled={addingPenalty}
          >
            {addingPenalty ? <LoadingSpinner /> : 'Strafe hinzufügen'}
          </button>
        </form>
      </Modal>
    </div>
