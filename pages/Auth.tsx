...
const Auth: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Nach Registrierung versuchen wir den Login, um zu sehen ob Mail-Bestätigung aktiv ist
        try {
          await signInWithEmail(email, password);
        } catch (loginErr: any) {
          // Spezifische Prüfung auf E-Mail Bestätigung
          if (loginErr.message?.toLowerCase().includes("confirm") || loginErr.status === 400) {
            setNeedsConfirmation(true);
            setSuccessInfo("Registrierung erfolgreich! Bitte bestätige deine E-Mail-Adresse, um dich einzuloggen.");
            setIsLogin(true);
          } else {
            throw loginErr;
          }
      ...
      if (msg.includes("Email not confirmed")) {
        setNeedsConfirmation(true);
        setError("Deine E-Mail wurde noch nicht bestätigt.");
  ...
  const handleResendMail = async () => {
    ...
    try {
      ...
      setSuccessInfo("Bestätigungs-E-Mail wurde erneut gesendet. Bitte schaue auch im Spam-Ordner nach.");
    } catch (err: any) {
      setError("Fehler beim Senden: " + (err.message || "Unbekannter Fehler"));
    } finally {
      setResending(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 sm:p-6 bg-slate-50 min-h-screen">
      <div className="w-full max-w-md bg-white p-6 sm:p-10 rounded-3xl shadow-xl border border-slate-100">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 p-4 rounded-2xl mb-6 font-bold text-xs border border-red-100 text-center animate-in fade-in zoom-in duration-300">
            {error}
            {needsConfirmation && (
              <button 
                onClick={handleResendMail} 
                disabled={resending}
                className="block mx-auto mt-2 text-indigo-600 underline hover:text-indigo-800 disabled:opacity-50"
              >
        ...
        {successInfo && (
          <div className="bg-green-50 text-green-700 p-4 rounded-2xl mb-6 font-bold text-xs border border-green-100 text-center animate-in fade-in slide-in-from-top-4 duration-500">
            {successInfo}
            {!isLogin && needsConfirmation && (
              <button 
                onClick={handleResendMail} 
                disabled={resending}
                className="block mx-auto mt-2 text-indigo-700 underline hover:text-indigo-900"
              >
                {resending ? 'Sende...' : 'Nichts erhalten? Erneut senden'}
              </button>
            )}
          </div>
        )}
        ...
        <p className="mt-8 text-center text-slate-500 font-medium text-sm">
          ...
          <button
            onClick={() => { setIsLogin(!isLogin); setError(null); setSuccessInfo(null); setNeedsConfirmation(false); }}
