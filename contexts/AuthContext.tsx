...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const refreshProfile = useCallback(async (currentUser: User) => {
    ...
    try {
      ...
      // Profilfehler verhindern keinen App-Start
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Sicherheits-Timeout: Ladekreis nach max. 8 Sekunden beenden
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100 text-center">
          <h2 className="text-xl font-bold text-red-600 mb-4">Verbindungsfehler</h2>
          <p className="text-slate-600 text-sm mb-6">{initError}</p>
          <button onClick={() => window.location.reload()} className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold">Neu laden</button>
        </div>
      </div>
    );
  }
